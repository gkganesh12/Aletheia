...
export default function IndustriesPage() {
  ...
  return (
    <PageTransition>
        ...
        <Container>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeSlug === industry.slug
                      ? "bg-[var(--color-accent-400)]/15 text-ink shadow-[0_0_20px_rgba(0,212,255,0.1)]"
                      : "bg-surface text-muted hover:bg-primary-900 hover:text-muted",
                  )}
                >
          ...
          <div ref={detailRef} className="mx-auto mt-12 max-w-6xl scroll-mt-24">
            <AnimatePresence mode="wait">
              ...
              >
                {/* Main card */}
                <div className="overflow-hidden rounded-3xl border border-ink/[0.08] bg-surface">
                  {/* Header row */}
                  <div className="border-b border-ink/[0.06] px-8 py-8 lg:px-12 lg:py-10">
                  ...
                  <div className="grid grid-cols-3 divide-x divide-ink/[0.06] border-b border-ink/[0.06]">
                    {active.stats.map((stat) => (
                      <div
                        key={stat.label}
                        className="px-6 py-6 text-center lg:px-8 lg:py-8"
                      >
                  ...
                  <div className="grid gap-0 lg:grid-cols-2">
                    {/* Challenges */}
                    <div className="border-b border-ink/[0.06] px-8 py-8 lg:border-b-0 lg:border-r lg:px-12 lg:py-10">
                    ...
                    <div className="border-b border-ink/[0.06] px-8 py-8 lg:px-12 lg:py-10">
                      <div className="flex items-center gap-2.5 mb-5">
                        <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[var(--color-accent-400)]/10">
                          <svg
                            className="h-3.5 w-3.5 text-[var(--color-accent-400)]"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2"
                    ...
                    <div className="flex flex-wrap gap-2.5">
                      {active.useCases.map((useCase, i) => (
                        <motion.span
                          ...
                          initial="hidden"
                          animate="visible"
                          className="rounded-full border border-ink/[0.08] bg-surface px-4 py-2 text-xs font-medium text-muted transition-colors hover:border-[var(--color-accent-400)]/20 hover:text-muted"
                        >
                          {useCase}
                        </motion.span>
                      ))}
      ...
      <section className="border-t border-ink/[0.04] py-20 lg:py-28">
        <Container>
          <AnimatedSection>
            <SectionHeading
            ...
            {industries.map((industry, i) => (
              <AnimatedSection key={industry.slug} delay={i * 0.08}>
                <motion.button
                  ...
                  className="group relative block w-full overflow-hidden rounded-2xl border border-ink/[0.08] bg-surface p-6 text-left transition-all duration-300 hover:border-[var(--color-accent-400)]/25 hover:bg-surface"
                  whileHover={{ y: -4 }}
                ...
                <GlassPanel className="group h-full rounded-2xl p-7 transition-all duration-300 hover:border-[var(--color-accent-400)]/20">
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {cap.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-md border border-ink/[0.06] bg-surface px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-muted"
                      >
                        {tag}
                      </span>
                    ))}
        ...
        <Container>
          <AnimatedSection>
            <div className="mx-auto max-w-3xl text-center">
              ...
              <h2 className="mt-6 text-3xl font-bold leading-tight text-ink md:text-4xl lg:text-5xl">
                Every industry has its own{" "}
                <span className="text-muted">
                  language, regulations, and risks.
                </span>
              </h2>
              <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-muted">
                ...
                workflows — and deliver solutions that actually work in
                production.
              </p>
            </div>
          </AnimatedSection>
